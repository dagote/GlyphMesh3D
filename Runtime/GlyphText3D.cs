...
public class GlyphText3D : MonoBehaviour
{
    ...
    [Header("Settings")]
    public GlyphText3DSettings settings;
    ...
    // Active settings getters
    private TMP_FontAsset ActiveFontAsset => settings != null ? settings.fontAsset : fontAsset;
    private float ActiveSimplifyArcLength => settings != null ? settings.simplifyArcLength : simplifyArcLength;
    private float ActiveCornerAngleThreshold => settings != null ? settings.cornerAngleThreshold : cornerAngleThreshold;
    private float ActivePostDpEpsilon => settings != null ? settings.postDpEpsilon : postDpEpsilon;
    private float ActiveExtrusionWidth => extrusionWidth;
    ...
    private static void CreateGlyphText3D()
    {
        ...
        GlyphText3D glyphText = go.AddComponent<GlyphText3D>();
    ...
    private void GenerateMeshForText()
    {
        ...
        {
            if (!ActiveFontAsset.characterLookupTable.TryGetValue(c, out TMPro.TMP_Character glyphChar))
                continue;
            ...
            Texture2D atlasTexture = ActiveFontAsset.atlasTexture;
            ...
            foreach (var chain in chains)
            {
                ...
                var simplified = SimplifyHybrid(pts, ActiveSimplifyArcLength, ActiveCornerAngleThreshold, ActivePostDpEpsilon);
    ...
    {
        if (ActiveFontAsset.glyphLookupTable != null && ActiveFontAsset.glyphLookupTable.ContainsKey(glyphChar.glyphIndex))
        {
            return ActiveFontAsset.glyphLookupTable[glyphChar.glyphIndex];
        }
        return ActiveFontAsset.glyphTable.FirstOrDefault(g => g.index == glyphChar.glyphIndex);
    }
    ...
    private List<Vector2> SmoothPathPreserveCorners(List<Vector2> points, int iterations, float tension)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            isCorner[i] = angle >= ActiveCornerAngleThreshold;
        }
        ...
        try
        {
            ...
            if (extrusionProfile != null && extrusionProfile.KeyframeCount > 0)
            {
                ...
                for (int i = 0; i < keyframeCount; i++)
                {
                    ...
                    float curveOffset = curveValue * curveValue * curveValue * ActiveExtrusionWidth;
...
#if UNITY_EDITOR
[CustomEditor(typeof(GlyphText3D))]
public class GlyphText3DEditor : Editor
{
    ...
    {
        GlyphText3D glyphText = target as GlyphText3D;
